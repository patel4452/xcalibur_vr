...
$gf(document).ready(function () {
	$gf('body').on('click', '#cplus-refresh-google-fonts', function () {
		...
		$gf.post(ajaxurl, data, function(response) {
			...
			$gf('#cplus-refresh-google-fonts').next('.spinner').css({'display':'none'});
			setTimeout(function(){
...
function cplus_get_google_fonts()
{
	...
	$gf.post(ajaxurl, data, function(response) {
		...
		{
			var ghtml = '<div class="gfont">It seems you don\'t have any Google Fonts yet. But you can download them now with <a href="javascript:void(0)" id="cplus-refresh-google-fonts">just a click.</a> <span class="spinner"></span></div>';
